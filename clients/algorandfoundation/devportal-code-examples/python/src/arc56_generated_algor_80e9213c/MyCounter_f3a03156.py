...
_APP_SPEC_JSON = r"""{"arcs": [22, 28], "bareActions": {"call": [], "create": ["NoOp"]}, "methods": [{"actions": {"call": ["OptIn"], "create": []}, "args": [], "name": "optIn", "returns": {"type": "void"}, "desc": "Initialize the counter when an account opts in", "events": [], "readonly": false, "recommendations": {}}, {"actions": {"call": ["NoOp"], "create": []}, "args": [], "name": "incrementMyCounter", "returns": {"type": "uint64", "desc": "The new counter value"}, "desc": "Increment the counter for the sender and return its new value", "events": [], "readonly": false, "recommendations": {}}], "name": "MyCounter", "state": {"keys": {"box": {}, "global": {}, "local": {"myCounter": {"key": "bXlfY291bnRlcg==", "keyType": "AVMString", "valueType": "AVMUint64"}}}, "maps": {"box": {}, "global": {}, "local": {}}, "schema": {"global": {"bytes": 0, "ints": 0}, "local": {"bytes": 0, "ints": 1}}}, "structs": {}, "desc": "A contract that maintains a per-account counter in local state\nAccounts must opt in to use the counter", "events": [], "networks": {}, "sourceInfo": {"approval": {"pcOffsetMethod": "none", "sourceInfo": [{"pc": [86], "errorMessage": "Account must opt in to contract first"}, {"pc": [73], "errorMessage": "OnCompletion must be NoOp && can only call when creating"}, {"pc": [51], "errorMessage": "OnCompletion must be NoOp && can only call when not creating"}, {"pc": [62], "errorMessage": "OnCompletion must be OptIn && can only call when not creating"}, {"pc": [92, 106], "errorMessage": "check LocalState exists"}]}, "clear": {"pcOffsetMethod": "none", "sourceInfo": []}}}"""
...
class _LocalState:
    ...
    def my_counter(self) -> int:
        """Get the current value of the myCounter key in local_state state"""
        value = self.app_client.state.local_state(self.address).get_value("myCounter")
...
class MyCounterClient:
    ...
    def decode_return_value(
        self,
        method: typing.Literal["incrementMyCounter()uint64"],
