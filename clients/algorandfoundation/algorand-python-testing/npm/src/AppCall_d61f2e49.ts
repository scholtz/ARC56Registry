...
export class AppCallClient {
   ...
   */
  decodeReturnValue<TSignature extends AppCallNonVoidMethodSignatures>(method: TSignature, returnValue: ABIReturn | undefined) {
  ...
  readonly params = {
     ...
     */
    someValue: (params: CallParams<AppCallArgs['obj']['some_value()uint64'] | AppCallArgs['tuple']['some_value()uint64']> & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
  ...
  readonly send = {
     ...
     */
    someValue: async (params: CallParams<AppCallArgs['obj']['some_value()uint64'] | AppCallArgs['tuple']['some_value()uint64']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
  ...
  public newGroup(): AppCallComposer {
    const client = this
    ...
    return {
       ...
       */
      someValue(params: CallParams<AppCallArgs['obj']['some_value()uint64'] | AppCallArgs['tuple']['some_value()uint64']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
        promiseChain = promiseChain.then(async () => composer.addAppCallMethodCall(await client.params.someValue(params)))
        resultMappers.push((v) => client.decodeReturnValue('some_value()uint64', v))
        return this
      },
      /**
...
export type AppCallComposer<TReturns extends [...any[]] = []> = {
   ...
   */
  simulate(): Promise<AppCallComposerResults<TReturns> & { simulateResponse: modelsv2.SimulateResponse }>
  simulate(options: SkipSignaturesSimulateOptions): Promise<AppCallComposerResults<TReturns> & { simulateResponse: modelsv2.SimulateResponse }>
  simulate(options: RawSimulateOptions): Promise<AppCallComposerResults<TReturns> & { simulateResponse: modelsv2.SimulateResponse }>
  /**
   * Sends the transaction group to the network and returns the results
   */
  send(params?: SendParams): Promise<AppCallComposerResults<TReturns>>
}
export type AppCallComposerResults<TReturns extends [...any[]]> = Expand<SendAtomicTransactionComposerResults & {
  returns: TReturns
}>
