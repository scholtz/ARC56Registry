...
export const APP_SPEC: Arc56Contract = {"name":"PersonalBank","structs":{},"methods":[{"name":"deposit","args":[{"type":"string","name":"github_handle"},{"type":"pay","name":"pay_txn","desc":"The payment transaction containing deposit information"}],"returns":{"type":"uint64","desc":"The total amount deposited by the sender after this transaction (as UInt64)"},"actions":{"create":[],"call":["NoOp"]},"readonly":false,"desc":"Deposits funds into the personal bank\nThis method accepts a payment transaction and records the deposit amount in the sender's BoxMap. If the sender already has a deposit, the amount is added to their existing balance.","events":[],"recommendations":{}},{"name":"withdraw","args":[],"returns":{"type":"uint64","desc":"The amount withdrawn (as UInt64)"},"actions":{"create":[],"call":["NoOp"]},"readonly":false,"desc":"Withdraws all funds from the sender's account\nThis method transfers the entire balance of the sender's account back to them, and resets their balance to zero. The sender must have a deposit to withdraw.","events":[],"recommendations":{}}],"arcs":[22,28],"networks":{},"state":{"schema":{"global":{"ints":0,"bytes":0},"local":{"ints":0,"bytes":0}},"keys":{"global":{},"local":{},"box":{"github_box":{"keyType":"AVMBytes","valueType":"AVMString","key":"Z2l0aHVi"}}},"maps":{"global":{},"local":{},"box":{"depositors":{"keyType":"address","valueType":"uint64","prefix":""}}}},"bareActions":{"create":["NoOp"],"call":[]},"sourceInfo":{"approval":{"sourceInfo":[{"pc":[131],"errorMessage":"Deposit amount must be greater than zero"},{"pc":[182],"errorMessage":"No deposits found for this account"},{"pc":[50,67],"errorMessage":"OnCompletion is not NoOp"},{"pc":[125],"errorMessage":"Receiver must be the contract address"},{"pc":[105],"errorMessage":"can only call when creating"},{"pc":[53,70],"errorMessage":"can only call when not creating"},{"pc":[150,162],"errorMessage":"check self.depositors entry exists"},{"pc":[86],"errorMessage":"transaction type is pay"}],"pcOffsetMethod":"none"},"clear":{"sourceInfo":[],"pcOffsetMethod":"none"}},"events":[]} as unknown as Arc56Contract
...
export type PersonalBankArgs = {
   ...
   */
  tuple: {
    'deposit(string,pay)uint64': [githubHandle: string, payTxn: AppMethodCallTransactionArgument]
    'withdraw()uint64': []
  }
}
...
export type PersonalBankTypes = {
  ...
  methods:
    & Record<'deposit(string,pay)uint64' | 'deposit', {
      argsObj: PersonalBankArgs['obj']['deposit(string,pay)uint64']
      argsTuple: PersonalBankArgs['tuple']['deposit(string,pay)uint64']
      /**
       * The total amount deposited by the sender after this transaction (as UInt64)
       */
      returns: PersonalBankReturns['deposit(string,pay)uint64']
    }>
...
export abstract class PersonalBankParamsFactory {
  ...
  * Deposits funds into the personal bank
   ...
   */
  static deposit(params: CallParams<PersonalBankArgs['obj']['deposit(string,pay)uint64'] | PersonalBankArgs['tuple']['deposit(string,pay)uint64']> & CallOnComplete): AppClientMethodCallParams & CallOnComplete {
    return {
      ...params,
      method: 'deposit(string,pay)uint64' as const,
      args: Array.isArray(params.args) ? params.args : [params.args.githubHandle, params.args.payTxn],
    }
  }
  /**
...
export class PersonalBankClient {
  ...
  readonly params = {
    ...
    /**
     * Makes a call to the PersonalBank smart contract using the `deposit(string,pay)uint64` ABI method.
     *
    * Deposits funds into the personal bank
    This method accepts a payment transaction and records the deposit amount in the sender's BoxMap. If the sender already has a deposit, the amount is added to their existing balance.
     ...
     */
    deposit: (params: CallParams<PersonalBankArgs['obj']['deposit(string,pay)uint64'] | PersonalBankArgs['tuple']['deposit(string,pay)uint64']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
  ...
  readonly send = {
    ...
    * Deposits funds into the personal bank
     ...
     */
    deposit: async (params: CallParams<PersonalBankArgs['obj']['deposit(string,pay)uint64'] | PersonalBankArgs['tuple']['deposit(string,pay)uint64']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      const result = await this.appClient.send.call(PersonalBankParamsFactory.deposit(params))
      return {...result, return: result.return as unknown as (undefined | PersonalBankReturns['deposit(string,pay)uint64'])}
    },
  ...
  state = {
    ...
    box: {
      ...
      getAll: async (): Promise<Partial<Expand<BoxKeysState>>> => {
        ...
        return {
          githubBox: result.github_box,
        }
      },
      /**
       * Get the current value of the github_box key in box state
       */
      githubBox: async (): Promise<string | undefined> => { return (await this.appClient.state.box.getValue("github_box")) as string | undefined },
      /**
  ...
  public newGroup(): PersonalBankComposer {
    ...
    return {
      /**
       * Add a deposit(string,pay)uint64 method call against the PersonalBank contract
       */
      deposit(params: CallParams<PersonalBankArgs['obj']['deposit(string,pay)uint64'] | PersonalBankArgs['tuple']['deposit(string,pay)uint64']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
...
export type PersonalBankComposer<TReturns extends [...any[]] = []> = {
  ...
  * Deposits funds into the personal bank
   ...
   */
  deposit(params?: CallParams<PersonalBankArgs['obj']['deposit(string,pay)uint64'] | PersonalBankArgs['tuple']['deposit(string,pay)uint64']>): PersonalBankComposer<[...TReturns, PersonalBankReturns['deposit(string,pay)uint64'] | undefined]>
